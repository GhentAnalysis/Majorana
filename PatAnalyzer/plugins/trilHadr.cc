...
trilHadr::trilHadr(const edm::ParameterSet & iConfig) :
...
void trilHadr::beginJob()
{
    ...
    */
    ...
    jecUnc = new JetCorrectionUncertainty("/user/ikhvastu/CMSSW_8_0_5/src/Majorana/PatAnalyzer/jetfiles/Spring16_25nsV6_MC_Uncertainty_AK4PFchs.txt");
...
void trilHadr::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void trilHadr::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
        ...
        */
        ...
        for(unsigned int i_Trig = 0; i_Trig < n_Triggers; ++i_Trig ) {
            if (trigResults.product()->accept(i_Trig)) {
                ...
                for (int i=0; i!=6; ++i) {
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNamesHadr[j][i])){
                            _triggers2l[j][i] = 1;
                    ...
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNamesHadrBkp[j][i])){
                ...
                for (int i=0; i!=2; ++i) {
                    if (TrigPath.Contains(_triggersCSbNamesHadr[i])){
                        _triggersCSb[i] = 1;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < sMu.size() ;i++ ){
        ...
        _isloose[leptonCounter] = good && (_3dIPsig[leptonCounter] < 4);
        _istight[leptonCounter] = good && (_3dIPsig[leptonCounter] < 4);
    ...
    std::cout << "Electron selection started" << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){
        ...
        std::cout << "Before Loose selection passed : " << iE->pt() << " " << passedMVA << " " << _trigEmulator[leptonCounter] << " " << (_miniisolation[leptonCounter][0] < 0.4) << " "  << (_hitsNumber[leptonCounter] == 0)  << " " << (!_vtxFitConversion[leptonCounter]) << " " << (_3dIPsig[leptonCounter] < 8) << endl;
        ...
        _isloose[leptonCounter] = _hitsNumber[leptonCounter] == 0 && (_3dIPsig[leptonCounter] < 4);
        _istight[leptonCounter] = passedMVA && _hitsNumber[leptonCounter] == 0 && (_3dIPsig[leptonCounter] < 4);
...
*/
    ...
    */
    ...
    bool cleanLeptons[10];
    for(int i = 0; i < _nLeptons; i++){
        cleanLeptons[i] = false;
    }
...
void trilHadr::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void trilHadr::fillCloseJetVars(const int leptonCounter, Vertex::Point PV) {
...
void trilHadr::matchCloseJet(const int leptonCounter) {
...
void trilHadr::fillIsoMCVars(const int leptonCounter) {
...
void trilHadr::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void trilHadr::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
