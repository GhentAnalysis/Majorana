...
onel::onel(const edm::ParameterSet & iConfig) :
...
void onel::beginJob()
{
    ...
    outputTree->Branch("_lchargemc", &_lchargemc, "_lchargemc[6]/D");
    ...
    */
    ...
    outputTree->Branch("_isolationDB", &_isolationDB, "_isolationDB[6]/D");
    ...
    */
    ...
    outputTree->Branch("_isPromptFinalState", &_isPromptFinalState, "_isPromptFinalState[6]/O");
    outputTree->Branch("_fromHardProcessFinalState", &_fromHardProcessFinalState, "_fromHardProcessFinalState[6]/O");
    ...
    outputTree->Branch("_isvetoIDCutBased", &_isvetoIDCutBased, "_isvetoIDCutBased[6]/O");
    ...
    outputTree->Branch("_trigEmulator", &_trigEmulator, "_trigEmulator[6]/O");
    outputTree->Branch("_isotrigEmulator", &_isotrigEmulator, "_isotrigEmulator[6]/O");
    ...
    outputTree->Branch("_chargeConst", &_chargeConst, "_chargeConst[6]/O");
    outputTree->Branch("_hitsNumber", &_hitsNumber, "_hitsNumber[6]/I");
    outputTree->Branch("_vtxFitConversion", &_vtxFitConversion, "_vtxFitConversion[6]/O");
    ...
    */
    ...
    outputTree->Branch("_jetFlavour", &_jetFlavour, "_jetFlavour[20]/I");
    outputTree->Branch("_jetE", &_jetE, "_jetE[20]/D");
    outputTree->Branch("_csv", &_csv, "_csv[20]/D");
    outputTree->Branch("_jetDeltaR", &_jetDeltaR, "_jetDeltaR[20][6]/D");

    // JEC
    outputTree->Branch("_jecUnc", &_jecUnc, "_jecUnc[20]/D");
    outputTree->Branch("_jetPtUp", &_jetPtUp, "_jetPtUp[20]/D");
    outputTree->Branch("_jetPtDown", &_jetPtDown, "_jetPtDown[20]/D");

    // JER
    outputTree->Branch("_matchedjetPt", &_matchedjetPt, "_matchedjetPt[20]/D");
    outputTree->Branch("_matchedjetEta", &_matchedjetEta, "_matchedjetEta[20]/D");
    outputTree->Branch("_matchedjetPhi", &_matchedjetPhi, "_matchedjetPhi[20]/D");
    outputTree->Branch("_matchedjetE", &_matchedjetE, "_matchedjetE[20]/D");
    outputTree->Branch("_matchedjetM", &_matchedjetM, "_matchedjetM[20]/D");
    outputTree->Branch("_matchGjet", &_matchGjet, "_matchGjet[20]/O");

    // b-tag SF
    outputTree->Branch("_btagSF", &_btagSF, "_btagSF[19][20]/D");
    ...
    if (isData)
        fMetCorrector = new OnTheFlyCorrections("Summer15_25nsV7_DATA", isData); //isData = true
    else
        fMetCorrector = new OnTheFlyCorrections("Summer15_25nsV7_MC", isData); //isData = true
    ...
    */
    ...
    jecUnc = new JetCorrectionUncertainty("/localgrid/ikhvastu/CMSSW_7_4_16/src/SUSYAnalyzer/PatAnalyzer/jetfiles/Summer15_25nsV7_MC_Uncertainty_AK4PFchs.txt");
...
void onel::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void onel::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
    ...
    */
     ...
     */
     ...
     if( !trigResults.failedToGet() ) {
        ...
        for(unsigned int i_Trig = 0; i_Trig < n_Triggers; ++i_Trig ) {
            if (trigResults.product()->accept(i_Trig)) {
                ...
                //std::cout << "Trigger path: " << TrigPath << std::endl;
        /*
                if( TrigPath.Contains("HLT_Mu8_v"))
                    _trigMu8 = true;
                if( TrigPath.Contains("HLT_Mu17_v"))
                    _trigMu17 = true;
                if( TrigPath.Contains("HLT_Mu8_TrkIsoVVL_v"))
                    _trigMu8iso = true;
                if( TrigPath.Contains("HLT_Mu17_TrkIsoVVL_v"))
                    _trigMu17iso = true;
                if( TrigPath.Contains("HLT_Ele12_CaloIdM_TrackIdM_PFJet30_v"))
                    _trigEle12 = true;
                if( TrigPath.Contains("HLT_Ele12_CaloIdL_TrackIdL_IsoVL_PFJet30_v"))
                    _trigEle12iso = true;
                if( TrigPath.Contains("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ"))
                    _trigDiMuIso = true;
                if( TrigPath.Contains("Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_v"))
                    _trigDiMuTkIso = true;
                if( TrigPath.Contains("HLT_Mu8_TrkIsoVVL_Ele17_CaloIdL_TrackIdL_IsoVL"))
                    _trigMu8Ele23Iso = true;
                if( TrigPath.Contains("HLT_Mu17_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL"))
                    _trigMu23Ele12Iso = true;
                if( TrigPath.Contains("HLT_Ele17_Ele12_CaloIdL_TrackIdL_IsoVL_DZ"))
                    _trigEle23Ele12Iso = true;
                if( TrigPath.Contains("DoubleMu8_Mass8_PFHT300"))
                    _trigDoubleMu8 = true;
                if( TrigPath.Contains("Mu8_Ele8_CaloIdM_TrackIdM_Mass8_PFHT300"))
                    _trigMu8Ele8 = true;
                if( TrigPath.Contains("DoubleEle8_CaloIdM_TrackIdM_Mass8_PFHT300"))
                    _trigDoubleEle8 = true;
                if( TrigPath.Contains("TripleMu_12_10_5"))
                    _trigTripleMu = true;
                if( TrigPath.Contains("DiMu9_Ele9_CaloIdL_TrackIdL"))
                    _trigTripleDiMu9Ele9 = true;
                if( TrigPath.Contains("Mu8_DiEle12_CaloIdL_TrackIdL"))
                    _trigTripleMu8DiEle12 = true;
                if( TrigPath.Contains("Ele16_Ele12_Ele8_CaloIdL_TrackIdL"))
                    _trigTripleEle16Ele12Ele8 = true;
            */
                ...
                /*
                 if (Sample!="ElectronsMC") {
                    std::pair<std::vector<std::pair<std::string,int> >,int> detailedPrescaleInfo = hltConfig_.prescaleValuesInDetail(iEvent, iEventSetup, triggerNames.triggerName(i_Trig));
                    ...
                    for( size_t varind = 0; varind < detailedPrescaleInfo.first.size(); varind++ ){
                        ...
                        //std::cout<<"First info about prescale: " << detailedPrescaleInfo.first.at(varind).first<<" "<<detailedPrescaleInfo.first.at(varind).second<<std::endl;
                    }
                    ...
                    if (minind < l1prescalevals.size()) {
                        //std::cout<< "Second info about prescale: " << TrigPath<<" "<<"" <<" "<<triggerPrescales->getPrescaleForIndex(i_Trig)<<" "<<detailedPrescaleInfo.second * l1prescalevals.at(minind)<<" "<<detailedPrescaleInfo.second<<" "<<l1prescalevals.at(minind)<<std::endl;
                        //std::cout<< "Second info about prescale: " << TrigPath<<" "<<"" <<" "<<detailedPrescaleInfo.second * l1prescalevals.at(minind)<<" "<<detailedPrescaleInfo.second<<" "<<l1prescalevals.at(minind)<<std::endl;
                        prescaleHLTL1 = detailedPrescaleInfo.second * l1prescalevals.at(minind);
                    }
                ...
                for (int i=0; i!=6; ++i) {
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNames4[j][i])){
                            _triggers2l[j][i] = 1;
                    ...
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNames4Bkp[j][i])){
                ...
                for (int i=0; i!=2; ++i) {
                    if (TrigPath.Contains(_triggersCSbNames4[i])){
                        _triggersCSb[i] = 1;
     ...
     //if((!_triggersCS[2][0]) && (!_triggersCS[2][1]) && (!_triggersCS[3][1])) return; // was applied to data, Wjets, DY, ttbarFXFX, 29 Jan 2016
     //std::cout << "Trigger requirement passed? " << _triggersCS[3][1] << std::endl;
     //if(!_triggersCS[3][1]) return; // was applied to data, Wjets, DY, ttbarFXFX, 29 Jan 2016
    
     //std::cout << "Trigger Name: " << _triggersCSNames4[3][1] << std::endl;
    ...
    */
    ...
    iEvent.getByLabel(edm::InputTag("fixedGridRhoFastjetAll","") , rhoJECJets);//kt6PFJets
    ...
    int leptonCounter = 0;
    //std::cout << "Muon selection started, muon candidante: " << sMu.size() << std::endl;
    for(unsigned int i = 0 ; i < sMu.size() ;i++ ){
        ...
        _isloose[leptonCounter] = good;
        _istight[leptonCounter] = good;

        //std::cout<< "Muon tight selection passed -> " << _istight[leptonCounter] << "; miniiso -> " << _miniisolation[leptonCounter][0] << std::endl;

        if (!_istight[leptonCounter]) continue;
        //if(_miniisolation[leptonCounter][0] > 1.0) continue;
        //if(_3dIPsig[leptonCounter])>4.
        if(_isolationDB[leptonCounter] > 1.0 ) continue; // ttZ fake rate calculation // 21 Jan 2016
    ...
    _nMu = leptonCounter;
     ...
     //std::cout << "Electron selection started, ele candidates: " << thePatElectrons->size() << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){

        if (leptonCounter == 6) continue;

        const auto iE = thePatElectrons->ptrAt(i);
        //std::cout << "Pat electron Pt/Eta/Phi: " << iE->pt() << " " << iE->superCluster()->eta() << " " << iE->phi() << std::endl;
        ...
        if (TMath::Abs(iE->eta()) < 0.8 ) {
            passedMVA = _mvaValue[leptonCounter]> looseMVA[0][0];
        } else if (TMath::Abs(iE->eta()) < 1.479 ) {
            passedMVA = _mvaValue[leptonCounter]> looseMVA[1][0];
        } else {
            passedMVA = _mvaValue[leptonCounter]> looseMVA[2][0];
        }
        */
        ...
        if(fabs(iE->superCluster()->eta()) < 1.479){
            _isloose[leptonCounter] = (ooEmooP < 0.102) && ( hOverE < 0.104) && (fabs(dEtaIn) < 0.0105) && (fabs(dPhiIn) < 0.115) && (full5x5_sigmaIetaIeta < 0.0103) &&   (fabs(_ipPV[leptonCounter]) < 0.0261) && (fabs(_ipZPV[leptonCounter]) < 0.41) && (_hitsNumber[leptonCounter] <= 2) && (!_vtxFitConversion[leptonCounter]);
        }
        else{
            _isloose[leptonCounter] = (ooEmooP < 0.126) && ( hOverE < 0.0897) && (fabs(dEtaIn) < 0.00814) && (fabs(dPhiIn) < 0.182) && (full5x5_sigmaIetaIeta < 0.0301) && (fabs(_ipPV[leptonCounter]) < 0.118) && (fabs(_ipZPV[leptonCounter]) < 0.822) && (_hitsNumber[leptonCounter] <= 1) && (!_vtxFitConversion[leptonCounter]);
        }
        //std::cout << "Electron eta: " << iE->superCluster()->eta() << std::endl;
        //std::cout << "Components of CB: " << ooEmooP << " " << hOverE << " " << fabs(dEtaIn) << " " << fabs(dPhiIn) << " " << full5x5_sigmaIetaIeta << " " <<  fabs(_ipPV[leptonCounter]) << " " << fabs(_ipZPV[leptonCounter]) << " " << _hitsNumber[leptonCounter] << " " << _vtxFitConversion[leptonCounter] << std::endl;
        //std::cout << "Passed loose definition: " << _isloose[leptonCounter] << std::endl;
        if(!_isloose[leptonCounter]) continue;
        //std::cout << "This electron passed loose requirement" << std::endl; 
 
        //_isloose[leptonCounter] = ( _ipPV[leptonCounter]< 0.05) && (_ipZPV[leptonCounter]< 0.1);
        //**************************************************************************************
        // Assign "clean" flag to electrons
        //**************************************************************************************
        /* // Remove removed :) 15 Jan 2016
        bool Remove = false;
        for (unsigned int ii = 0 ; ii < sMu.size() ; ii++) {
            const pat::Muon *mu = sMu[ii];
            double miniisolation_loc = getPFIsolation(pfcands, dynamic_cast<const reco::Candidate *>(mu), 0.05, 0.2, 10., false, false, myRhoJets);
            if (miniisolation_loc < 0.4) {
        ...
        */
        ...
        //_istight[leptonCounter] = passedMVA && (_hitsNumber[leptonCounter] == 0);
        if(fabs(iE->superCluster()->eta()) < 1.479){
            _istight[leptonCounter] = (ooEmooP < 0.0174) && ( hOverE < 0.0876) && (fabs(dEtaIn) < 0.0103) && (fabs(dPhiIn) < 0.0336) && (full5x5_sigmaIetaIeta < 0.0101)  && (fabs(_ipPV[leptonCounter]) < 0.0118) && (fabs(_ipZPV[leptonCounter]) < 0.373) && (_hitsNumber[leptonCounter] <= 2) && (!_vtxFitConversion[leptonCounter]);
        }
        else{
            _istight[leptonCounter] = (ooEmooP < 0.0898) && ( hOverE < 0.0678) && (fabs(dEtaIn) < 0.00733) && (fabs(dPhiIn) < 0.114) && (full5x5_sigmaIetaIeta < 0.0283)  && (fabs(_ipPV[leptonCounter]) < 0.0739) && (fabs(_ipZPV[leptonCounter]) < 0.602) && (_hitsNumber[leptonCounter] <= 1) && (!_vtxFitConversion[leptonCounter]);
        }
        if(!_istight[leptonCounter]) continue;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < SelectedJets.size() ;i++ ){
        ...
        //std::cout << "Before correction pt of jet: " << uncPt << std::endl;
        ...
        for (int k=0; k!=_nLeptons; ++k) {
            if (_3dIPsig[k] > 4. ) continue;
            if (_flavors[k] == 0 && _miniisolation[k][0] > 0.4) continue;
            double dR1 = ((TLorentzVector *)_leptonP4->At(k))->DeltaR( jt );
            //std::cout << "jet cleaning: " << dR1 << std::endl;
            clean = clean && (dR1 > 0.4) ;
        }
        //if (!clean) continue; // Not cleaning jets with leptons 15 Jan 2016
...
void onel::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void onel::fillCloseJetVars(const int leptonCounter) {
...
void onel::matchCloseJet(const int leptonCounter) {
...
void onel::fillIsoMCVars(const int leptonCounter) {
...
void onel::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void onel::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
