...
eff13::eff13(const edm::ParameterSet & iConfig) :
...
void eff13::beginJob()
{
    ...
    outputTree->Branch("_lgenParticle", &_lgenParticle, "_lgenParticle[6]/O");
    ...
    outputTree->Branch("_activity", &_activity, "_activity[6]/D");
    outputTree->Branch("_miniisolationDB", &_miniisolationDB, "_miniisolationDB[6]/D");
    outputTree->Branch("_multiisolation", &_multiisolation, "_multiisolation[6][5]/O");
    outputTree->Branch("_ptrel", &_ptrel, "_ptrel[6]/D");
    outputTree->Branch("_ptrel_unc", &_ptrel_unc, "_ptrel_unc[6]/D");
    ...
    outputTree->Branch("_ismedium", &_ismedium, "_ismedium[6]/O");
    ...
    outputTree->Branch("_closeJetEtaAll", &_closeJetEtaAll, "_closeJetEtaAll[6]/D");
    outputTree->Branch("_closeJetPhiAll", &_closeJetPhiAll, "_closeJetPhiAll[6]/D");
    outputTree->Branch("_closeJetEAll", &_closeJetEAll, "_closeJetEAll[6]/D");
    outputTree->Branch("_closeJetMAll", &_closeJetMAll, "_closeJetMAll[6]/D");
    outputTree->Branch("_closeJetNconstAll", &_closeJetNconstAll, "_closeJetNconstAll[6]/D");
    outputTree->Branch("_closeJetCSVAll", &_closeJetCSVAll, "_closeJetCSVAll[6]/D");

    outputTree->Branch("_closeJetPtAll_unc", &_closeJetPtAll_unc, "_closeJetPtAll_unc[6]/D");
    ...
    outputTree->Branch("_RhoCN", &_RhoCN, "_RhoCN/D");
    outputTree->Branch("_RhoAll", &_RhoAll, "_RhoAll/D");
    ...
    if (isData)
        ...
        //fMetCorrector = new OnTheFlyCorrections("MCRUN2_74_V9", isData); // fastsim
        fMetCorrector = new OnTheFlyCorrections("Spring16_25nsV6_MC", isData); // fullsim
...
void eff13::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
        ...
        if( TheGenParticles.isValid() )
        {
            ...
            for(GenParticleCollection::const_reverse_iterator p = TheGenParticles->rbegin() ; p != TheGenParticles->rend() ; p++ )
            {
                ...
                if (id == 11 || id == 13){
                    ...
                    std::cout << "All mother of " << id << ": ";
                    //int n = p->numberOfDaughters(); 
                    std::vector<const GenParticle*> moms = GPM.getAllMothers(p);
                    for(uint i = 0 ; i < m.size(); i++ ){
                        int idmom = TMath::Abs(m[i]->pdgId()) ;
                        std::cout << idmom << " ";
                    }
                    std::cout << std::endl;
    ...
    */
    ...
    edm::Handle<double> JEC_rhoJets;
    iEvent.getByLabel(edm::InputTag("fixedGridRhoFastjetCentralNeutral","") , rhoJets);//kt6PFJets
    myRhoJets = *rhoJets;
    iEvent.getByLabel(edm::InputTag("fixedGridRhoFastjetAll","") , JEC_rhoJets);//kt6PFJets
    JECRhoJets = *JEC_rhoJets;
    //==================================
    ...
    for(unsigned int i = 0 ; i < sMu.size() ;i++ ){
        ...
        std::cout << "Muons pt/eta/phi: " << iM->pt() << " " << iM->eta() << " " << iM->phi() << std::endl;
        ...
        std::cout << "Activity: " << _activity[leptonCounter] << "; Miniiso: " << _miniisolation[leptonCounter][0] << std::endl;

        _miniisolationDB[leptonCounter] = getPFIsolationDB(pfcands, dynamic_cast<const reco::Candidate *>(iM), 0.05, 0.2, 10., false, false, myRhoJets);
        /*
        const reco::TrackRef innerTrack = iM->innerTrack();
        ...
        */
        ...
        _ismedium[leptonCounter] = good;
        _istight[leptonCounter] = good_tight;
        ...
        _lgenParticle[leptonCounter] = iM->genParticleRef().isNonnull();
        std::cout << "Gen particle exists? " << iM->genParticleRef().isNonnull() << std::endl;
    ...
    std::cout << "Electron selection started" << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){
        ...
        if (!effMVAElectronSelectorPassed(iE, _minPt1, PV, _looseD0E, _chargeConsistency, theConversions, BS, false)) continue;
        
        if (leptonCounter == 6) continue;
        std::cout << "Electron pt/eta/phi: " <<  iE->pt() << " " << iE->eta() << " " << iE->phi() << std::endl;
        ...
        _ipPVerr[leptonCounter] = iE->gsfTrack()->dxyError();
        
        _ipZPV[leptonCounter] = iE->gsfTrack()->dz(PV);
        _ipZPVerr[leptonCounter] = iE->gsfTrack()->dzError();
        ...
        std::cout << "Activity: " << _activity[leptonCounter] << "; Miniiso: " << _miniisolation[leptonCounter][0] << "; RelIso04: " << _isolation[leptonCounter] << std::endl;
        std::cout << "Comparison: " << _activity[leptonCounter] + _miniisolation[leptonCounter][0] << "; " << _isolation[leptonCounter] << std::endl;
       //**************************************************************************************
        ...
        */
        ...
        //std::cout << "Passed isolation:\t" << (_isolation[leptonCounter]<_relIsoCutE) << std::endl;
        ...
        */
        ...
        _lgenParticle[leptonCounter] = iE->genParticleRef().isNonnull();
        std::cout << "Gen particle exists? " << iE->genParticleRef().isNonnull() << std::endl;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < SelectedJets.size() ;i++ ){
        ...
        double corr = fMetCorrector->getJetCorrectionRawPt(uncPt, uncEta, JECRhoJets, SelectedJets[i]->jetArea(),_corrLevel);
        ...
        TLorentzVector jt; jt.SetPtEtaPhiM(_jetPt[_n_Jets],_jetEta[_n_Jets],_jetPhi[_n_Jets],_jetE[_n_Jets]);
        bool clean = true;
        ...
        _csv[_n_Jets] = SelectedJets[i]->bDiscriminator("combinedInclusiveSecondaryVertexV2BJetTags");
    ...
    /*
    for (int l0 = 0; l0<_nMu+_nEle; ++l0) {
        for (int l1 = l0+1; l1<_nMu+_nEle; ++l1) {
            if (_charges[l0] != _charges[l1]) {
                double mdiL = Mll_calc( *((TLorentzVector*)_leptonP4->At(l0)), *((TLorentzVector*)_leptonP4->At(l1)));
                if ((_flavors[l0] == _flavors[l1]) && (fabs(mdiL - 91) < deltaM)) {
                    //found = true;
                    _ossf[0] = true;
                    deltaM = fabs(mdiL - 91);
                    _mll[0] = mdiL;
    ...
    */
    ...
    std::cout<<_runNb<<" "<<_lumiBlock<<" "<<_eventNb<<" "<<_nMu<<" "<<_nEle<<" "<<_nTau<<" "<<_n_Jets<<" "<<_n_bJets<<std::endl;
    outputTree->Fill();
    /*if (_ossf[0] && fabs(_mll[0]-91) < 15 && _met > 40 && _mt[_indeces[2]] > 40) {
        std::cout<<_runNb<<" "<<_lumiBlock<<" "<<_eventNb<<" "<<_nMu<<" "<<_nEle<<" "<<_nTau<<" "<<_n_Jets<<" "<<_n_bJets
        <<" "<<_indeces[0]<<" "<<_indeces[1]<<" "<<_indeces[2]<<" "<<_mll[0]<<" "<<_mt[_indeces[2]]<<" "<<_met<<std::endl;
        //if (deltaM < 15)
    //}
    }*/
    //}
...
void eff13::fillMCVars(const GenParticle* mc, const int leptonCounter) {
    ...
    std::cout << "Gen matched lepton pt/eta/phi/status/pdgId: " << _lPtmc[leptonCounter] << " " << _lEtamc[leptonCounter] << " " << _lPhimc[leptonCounter] << " " << mc->status() << " " << TMath::Abs(mc->pdgId()) << std::endl;
    ...
    std::cout << "Origin/PromptFinalState/FromHardProcess: " << _originReduced[leptonCounter] << " " << _isPromptFinalState[leptonCounter] << " " << _fromHardProcessFinalState[leptonCounter] << std::endl;
...
void eff13::fillCloseJetVars(const int leptonCounter) {
    ...
    for(unsigned int k = 0 ; k < SelectedJetsAll.size() ;k++ ){
        ...
        double corr = fMetCorrector->getJetCorrectionRawPt( uncorrPt, (SelectedJetsAll[k]->correctedP4("Uncorrected")).Eta(), JECRhoJets, SelectedJetsAll[k]->jetArea(),"L1FastJet");
        double corr2 = fMetCorrector->getJetCorrectionRawPt( uncorrPt, (SelectedJetsAll[k]->correctedP4("Uncorrected")).Eta(), JECRhoJets, SelectedJetsAll[k]->jetArea(),_corrLevel);
        ...
        //pJet.SetPtEtaPhiE( _jetPtAll[k], _jetEtaAll[k], _jetPhiAll[k], _jetEAll[k]);
        //pJet_unc.SetPtEtaPhiE( uncPt, _jetEtaAll[k], _jetPhiAll[k], uncE);
...
void eff13::matchCloseJet(const int leptonCounter) {
...
void eff13::fillIsoMCVars(const int leptonCounter) {
...
void eff13::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void eff13::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
