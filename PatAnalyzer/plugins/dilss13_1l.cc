...
dilss13_1l::dilss13_1l(const edm::ParameterSet & iConfig) :
...
void dilss13_1l::beginJob()
{
    ...
    */
    ...
    if (isData)
        fMetCorrector = new OnTheFlyCorrections("Summer15_25nsV6_DATA", isData); //isData = true
    else
        fMetCorrector = new OnTheFlyCorrections("Summer15_25nsV6_MC", isData); //isData = true
...
void dilss13_1l::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void dilss13_1l::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
     ...
     if( !trigResults.failedToGet() ) {
        ...
        /*
            ...
            */
                ...
                for (int i=0; i!=6; ++i) {
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNames6[j][i])){
                            _triggers2l[j][i] = 1;
                    ...
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNames6Bkp[j][i])){
                ...
                for (int i=0; i!=2; ++i) {
                    if (TrigPath.Contains(_triggersCSbNames6[i])){
                        _triggersCSb[i] = 1;
    ...
    */
    ...
    _met = pfmet->pt();
    _met_phi = pfmet->phi();
    std::cout<< "met values: " <<_met<<"; "<<_met_phi<<std::endl;
    ...
    */
    ...
    //std::cout << "Electron selection started" << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){
        ...
        std::cout << "Loose selection: " << passedMVA << " " << _vtxFitConversion[leptonCounter] << " " << _hitsNumber[leptonCounter] << " " << _miniisolation[leptonCounter][0] << " " << _trigEmulator[leptonCounter] << std::endl;
...
void dilss13_1l::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void dilss13_1l::fillCloseJetVars(const int leptonCounter) {
...
void dilss13_1l::matchCloseJet(const int leptonCounter) {
...
void dilss13_1l::fillIsoMCVars(const int leptonCounter) {
...
void dilss13_1l::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void dilss13_1l::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
