...
dilss::dilss(const edm::ParameterSet & iConfig) :
...
void dilss::beginJob()
{
    ...
    */
    ...
    if (isData)
        fMetCorrector = new OnTheFlyCorrections("Fall15_25nsV2_DATA", isData); //isData = true
    else
        fMetCorrector = new OnTheFlyCorrections("Fall15_25nsV2_MC", isData); //isData = true
...
void dilss::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void dilss::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
    ...
    */
    ...
    /*
     ...
     /*           for (int i=0; i!=6; ++i) {
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNamesEWK[j][i])){
                            _triggers2l[j][i] = 1;
                    ...
                    for (int j=0; j!=2; ++j) {
                        if (TrigPath.Contains(_triggers2lNamesEWKBkp[j][i])){
                ...
                for (int i=0; i!=2; ++i) {
                    if (TrigPath.Contains(_triggersCSbNamesEWK[i])){
                        _triggersCSb[i] = 1;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < sMu.size() ;i++ ){
        ...
        _isloose[leptonCounter] = (_3dIPsig[leptonCounter] < 8);
        _istight[leptonCounter] = good && (_3dIPsig[leptonCounter] < 8);
    ...
    std::cout << "Electron selection started" << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){
        ...
        std::cout << "Before Loose selection passed : " << iE->pt() << " " << passedMVA << " " << _trigEmulator[leptonCounter] << " " << (_miniisolation[leptonCounter][0] > 0.4) << " " << (!_vtxFitConversion[leptonCounter]) << " " << (_3dIPsig[leptonCounter] > 8) << endl;
        ...
        if(_3dIPsig[leptonCounter] > 8) continue;

        std::cout << "Loose selection passed : " << iE->pt() << endl;
        ...
        _istight[leptonCounter] = passedMVA;
        ...
        cout << "electron pt/eta: " << _lPt[leptonCounter] << " " <<  _lEta[leptonCounter] << endl;
    ...
    for(unsigned int i = 0 ; i < sTau.size() ;i++ ){
        ...
        cout << "tau dz and dxy: " << TMath::Abs(Tau_dz(iTau->leadChargedHadrCand()->vertex(), *((TLorentzVector *)_leptonP4->At(leptonCounter)), PV)) << " " << TMath::Abs(_ipPV[leptonCounter]) << endl; 
        ...
        if(!(_decayModeFinding[leptonCounter] && _vlooseMVAold[leptonCounter])) continue;
...
void dilss::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void dilss::fillCloseJetVars(const int leptonCounter, Vertex::Point PV) {
    ...
    for(unsigned int k = 0 ; k < SelectedJetsAll.size() ;k++ ){
       ...
       if (ang < _closeJetAngAll[leptonCounter]) {
           ...
           for(int unsigned it=0; it!=SelectedJetsAll[_closeIndex[leptonCounter]]->numberOfDaughters(); ++it) {
             ...
             bool trackSelection =  trk.charge() != 0 && (TMath::Sqrt( deta*deta+dphi*dphi ) < 0.4) && (icand->fromPV() > 1) && (trk.hitPattern().numberOfValidHits()>=8) && (trk.hitPattern().numberOfValidPixelHits()>=2) && (trk.normalizedChi2()<5) && std::fabs(trk.dxy(PV))<0.2 && std::fabs(trk.dz(PV))<17;
...
void dilss::matchCloseJet(const int leptonCounter) {
...
void dilss::fillIsoMCVars(const int leptonCounter) {
...
void dilss::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void dilss::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
