...
trilSUSY::trilSUSY(const edm::ParameterSet & iConfig) :
...
void trilSUSY::beginJob()
{
    ...
    outputTree->Branch("_jetFlavour", &_jetFlavour, "_jetFlavour[20]/D");

    outputTree->Branch("_csv", &_csv, "_csv[20]/D");

    outputTree->Branch("_jetDeltaRloose", &_jetDeltaRloose, "_jetDeltaRloose[20]/D");
    outputTree->Branch("_jetDeltaR", &_jetDeltaR, "_jetDeltaR[20][10]/D");
...
void trilSUSY::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < sMu.size() ;i++ ){
        ...
        //bool good_tight = iM->isGlobalMuon() && iM->isPFMuon() && iM->globalTrack()->normalizedChi2() && iM->globalTrack()->hitPattern().numberOfValidMuonHits() > 0 && iM->numberOfMatchedStations() > 1 && fabs(iM->muonBestTrack()->dxy(PV)) < 0.2 && fabs(iM->muonBestTrack()->dz(PV)) < 0.5 && iM->innerTrack()->hitPattern().numberOfValidPixelHits() > 0 && iM->innerTrack()->hitPattern().trackerLayersWithMeasurement() > 5;
        //_ismediumID[leptonCounter] = good;
        ...
        _isloose[leptonCounter] = good_loose && fabs(_ipPV[leptonCounter])<_looseD0Mu && fabs(_ipZPV[leptonCounter]) < 0.1 && _miniisolation[leptonCounter][0] < 0.4;
        if(!_isloose[leptonCounter]) continue;
        
        _istight[leptonCounter] = _istightID[leptonCounter] && _3dIPsig[leptonCounter]<4.;
    ...
    std::cout << "Electron selection started" << std::endl;
     for (size_t i = 0; i < thePatElectrons->size(); ++i){
        ...
        _isloose[leptonCounter] = passedMVA && _trigEmulator[leptonCounter] && fabs(_ipPV[leptonCounter])<_looseD0E && fabs(_ipZPV[leptonCounter])<0.1 && _miniisolation[leptonCounter][0]<_relIsoCutEloose && _hitsNumber[leptonCounter] <= 1 && (!_vtxFitConversion[leptonCounter]);

        std::cout << "Electron loose info: " << passedMVA << " " << _trigEmulator[leptonCounter] << " " << (fabs(_ipPV[leptonCounter])<_looseD0E) << " " <<  (fabs(_ipZPV[leptonCounter])<0.1) << " " << (_miniisolation[leptonCounter][0]<_relIsoCutEloose) << " " << (_hitsNumber[leptonCounter] <= 1) << " " << (!_vtxFitConversion[leptonCounter]) << std::endl;
        std::cout << "hits nb: " << _hitsNumber[leptonCounter] << std::endl;
       ...
       //**************************************************************************************
        ...
        //**************************************************************************************
        bool Remove = false;
        for (int ii = 0 ; ii < _nMu ; ii++) {
            //TLorentzVector mu1; mu1.SetPtEtaPhiE(_lPt[ii], _lEta[ii], _lPhi[ii], _lE[ii]);
            TLorentzVector el; el.SetPtEtaPhiE(iE->pt(),iE->eta(),iE->phi(),iE->energy());
            //std::cout << "Muon parameters: "<<mu->pt()<<" "<<mu->eta()<<" "<<mu->phi()<<std::endl;
            float dR = ((TLorentzVector *)_leptonP4->At(ii))->DeltaR( el ) ;;
        ...
        _istight[leptonCounter] = ( passedMVA && _3dIPsig[leptonCounter] < 4 && _hitsNumber[leptonCounter] == 0 );
    ...
    for(unsigned int i = 0 ; i < SelectedJets.size() ;i++ ){
        ...
        _jetFlavour[_n_Jets] = SelectedJets[i]->partonFlavour();
     ...
     for (int ii = 0 ; ii < _nMu ; ii++) {
        ...
        for(int i = 0; i != _n_Jets; i++){
            TLorentzVector jt; jt.SetPtEtaPhiM(_jetPt[i],_jetEta[i],_jetPhi[i],_jetM[i]);
            float dR = mu1.DeltaR(jt);
            if( dR < _jetDeltaRloose[i] ) { //for sync
                _jetDeltaRloose[i] = dR; 
            }
        }
     }

     for (int ii = _nMu; ii < _nMu+_nEle; ++ii){
             
        TLorentzVector el1; el1.SetPtEtaPhiE(_lPt[ii], _lEta[ii], _lPhi[ii], _lE[ii]);

        for(int i = 0; i != _n_Jets; i++){
            TLorentzVector jt; jt.SetPtEtaPhiM(_jetPt[i],_jetEta[i],_jetPhi[i],_jetM[i]);
            float dR = el1.DeltaR( jt );
            if( dR < _jetDeltaRloose[i] ) { //for sync
                _jetDeltaRloose[i] = dR;
            }
        }
...
void trilSUSY::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void trilSUSY::fillCloseJetVars(const int leptonCounter) {
    ...
    for(unsigned int k = 0 ; k < SelectedJetsAll.size() ;k++ ){
        ...
        //pJet_unc.SetPtEtaPhiM( uncPt, _jetEtaAll[k], _jetPhiAll[k], uncE);
        ...
        if (ang < _closeJetAngAll[leptonCounter]) {
            ...
            //_closeJetPtAll_unc[leptonCounter] = uncPt;
            ...
            //_ptrel_unc[leptonCounter] = ((TLorentzVector *)_leptonP4->At(leptonCounter))->Perp(pJet_unc.Vect() - ((TLorentzVector *)_leptonP4->At(leptonCounter))->Vect());
    ...
    //std::cout << "MINI, PTRATIO, PTREL: " << _miniisolation[leptonCounter][0] << " " << _ptratio[leptonCounter] << " " << _ptrel[leptonCounter] <<std::endl;
}

void trilSUSY::matchCloseJet(const int leptonCounter) {
...
void trilSUSY::fillIsoMCVars(const int leptonCounter) {
...
void trilSUSY::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void trilSUSY::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
