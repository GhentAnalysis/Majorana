...
dilos::dilos(const edm::ParameterSet & iConfig) :
...
void dilos::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void dilos::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
    ...
    */
    ...
    for(unsigned int i = 0 ; i < sTau.size() ;i++ ){
        ...
        cout << "tau dz and dxy: " << TMath::Abs(Tau_dz(iTau->leadChargedHadrCand()->vertex(), *((TLorentzVector *)_leptonP4->At(leptonCounter)), PV)) << " " << TMath::Abs(_ipPV[leptonCounter]) << " " << _decayModeFinding[leptonCounter] << " " << _looseMVA_dR03[leptonCounter] << endl; 
...
void dilos::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void dilos::fillCloseJetVars(const int leptonCounter, Vertex::Point PV) {
...
void dilos::matchCloseJet(const int leptonCounter) {
...
void dilos::fillIsoMCVars(const int leptonCounter) {
...
void dilos::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void dilos::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
