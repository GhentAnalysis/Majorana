...
trilEWK::trilEWK(const edm::ParameterSet & iConfig) :
...
void trilEWK::beginJob()
{
    ...
    */
    ...
    if (isData)
        fMetCorrector = new OnTheFlyCorrections("Spring16_25nsV3_DATA", isData); //isData = true
    else
        fMetCorrector = new OnTheFlyCorrections("Spring16_25nsV3_MC", isData); //isData = true
...
void trilEWK::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void trilEWK::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
...
void trilEWK::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void trilEWK::fillCloseJetVars(const int leptonCounter, Vertex::Point PV) {
...
void trilEWK::matchCloseJet(const int leptonCounter) {
...
void trilEWK::fillIsoMCVars(const int leptonCounter) {
...
void trilEWK::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void trilEWK::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
