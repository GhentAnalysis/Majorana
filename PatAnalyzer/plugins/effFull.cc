...
effFull::effFull(const edm::ParameterSet & iConfig) :
...
void effFull::beginJob()
{
    ...
    outputTree->Branch("_nLeptons_truth", &_nLeptons_truth, "_nLeptons_truth/I");
    outputTree->Branch("_lPt_truth", &_lPt_truth, "_lPt_truth[_nLeptons_truth]/D");
    outputTree->Branch("_lEta_truth", &_lEta_truth, "_lEta_truth[_nLeptons_truth]/D");
    outputTree->Branch("_lPhi_truth", &_lPhi_truth, "_lPhi_truth[_nLeptons_truth]/D");
    outputTree->Branch("_lE_truth", &_lE_truth, "_lE_truth[_nLeptons_truth]/D");
    outputTree->Branch("_lpdg_truth", &_lpdg_truth, "_lpdg_truth[_nLeptons_truth]/D");
...
void effFull::beginRun(const edm::Run& iRun, edm::EventSetup const& iSetup)
{
    using namespace edm;
...
void effFull::analyze(const edm::Event& iEvent, const edm::EventSetup& iEventSetup)
{
    //bool islepton;
    ...
    if (Sample=="ElectronsMC") {
        ...
        if( TheGenParticles.isValid() )
        {
            ...
            for(GenParticleCollection::const_reverse_iterator p = TheGenParticles->rbegin() ; p != TheGenParticles->rend() ; p++ )
            {
                ...
                if(GPM.origin(&*p) == 0 && (p->status() == 1)){
                    _lPt_truth[leptonCounter_truth] = p->pt();
                    _lE_truth[leptonCounter_truth] = p->energy();
                    _lPhi_truth[leptonCounter_truth] = p->phi();
                    _lEta_truth[leptonCounter_truth] = p->eta();
                    _lpdg_truth[leptonCounter_truth] = p->pdgId();
                    leptonCounter_truth++;
                }
            }
            _nLeptons_truth = leptonCounter_truth;
            cout << endl;
            if (Gen0.E()!=0) {
                _genmet = Gen0.Pt();
...
void effFull::fillMCVars(const GenParticle* mc, const int leptonCounter) {
...
void effFull::fillCloseJetVars(const int leptonCounter, Vertex::Point PV) {
...
void effFull::matchCloseJet(const int leptonCounter) {
...
void effFull::fillIsoMCVars(const int leptonCounter) {
...
void effFull::fillRegVars(const pat::Jet *jet, double genpt, const pat::Muon* mu) {
...
void effFull::fillRegVars(const pat::Jet *jet, double genpt, const pat::Electron* mu) {
