                                                      ...
                                                      double v_muon_d0)
{
    ...
    for( std::vector<pat::Muon>::const_iterator mu = thePatMuons.begin() ; mu != thePatMuons.end() ; mu++ )
    {
        ...
        if ( !(mu->isPFMuon() || mu->isTrackerMuon()) ) continue;
...
bool tools::effMVAElectronSelectorPassed(const edm::Ptr<pat::Electron> el,
                                                                 ...
                                                                 bool tight)
{
    ...
    //double v_electron_dz = 0.1; // 0.1 for sync

    if( el->pt() < v_electron_pt ) return false;
    if( TMath::Abs(el->eta()) > v_electron_eta ) return false;
        
    const reco::GsfTrackRef gsfTrack = el->gsfTrack();
    if (!gsfTrack.isNonnull()) return false;
    //if( TMath::Abs(gsfTrack->dxy(PV)) > v_electron_d0  )  return false;
    //if( TMath::Abs(gsfTrack->dz(PV)) > v_electron_dz  ) return false;
        
    //if( bool_electron_chargeConsistency && !el->isGsfCtfScPixChargeConsistent() )  return false;
                                                                 ...
                                                                 bool tight)
{
    ...
    cout << "electron info before selection: " << el->pt() << " " << el->eta() << endl;
...

bool tools::ssbElectronSelectorPassed(const edm::Ptr<pat::Electron> el,
                                                              double v_electron_pt,
                                                              reco::Vertex::Point PV,
                                                              double v_electron_d0,
                                                              bool bool_electron_chargeConsistency,
                                                              ...
                                                              reco::BeamSpot::Point BS)
{
        ...
        if( el->pt() < v_electron_pt ) return false;
        if( TMath::Abs(el->eta()) > v_electron_eta ) return false;
        if( TMath::Abs(el->superCluster()->eta()) > 1.4442 && TMath::Abs(el->superCluster()->eta()) < 1.566 ) return false;

        if( TMath::Abs(el->superCluster()->eta()) < 1.479  ) {
            if( TMath::Abs(el->full5x5_sigmaIetaIeta()) > 0.010399 ) return false;
            if( TMath::Abs(el->deltaEtaSuperClusterTrackAtVtx()) > 0.007641 ) return false;
            if( TMath::Abs(el->deltaPhiSuperClusterTrackAtVtx()) > 0.32643  ) return false;
            if( TMath::Abs(el->hadronicOverEm())  > 0.060642  ) return false;  //recommended is 0.12 but HLT applies 0.1
            if( TMath::Abs(1.0/el->ecalEnergy() - el->eSuperClusterOverP()/el->ecalEnergy()) > 0.153897) return false;
            if( TMath::Abs(gsfTrack->dxy(PV)) > 0.011811) return false;
            if( TMath::Abs(gsfTrack->dz(PV)) > 0.070775) return false;
        
	    }
        else {
            if( TMath::Abs(el->full5x5_sigmaIetaIeta()) > 0.029524 ) return false;
            if( TMath::Abs(el->deltaEtaSuperClusterTrackAtVtx()) > 0.009285 ) return false;
            if( TMath::Abs(el->deltaPhiSuperClusterTrackAtVtx()) > 0.042447  ) return false;
            if( TMath::Abs(el->hadronicOverEm()) > 0.104263  )  return false;  //recommended is 0.12 but HLT applies 0.1
            if( TMath::Abs(1.0/el->ecalEnergy() - el->eSuperClusterOverP()/el->ecalEnergy()) > 0.137468)  return false;
            if( TMath::Abs(gsfTrack->dxy(PV)) > 0.051682)   return false;
            if( TMath::Abs(gsfTrack->dz(PV)) > 0.180720)  return false;
	    }
        ...
        */
        ...
        if( gsfTrack->hitPattern().numberOfHits(reco::HitPattern::MISSING_INNER_HITS) > 1 ) return false;
